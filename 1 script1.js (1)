...
function openMemoryLockPinModal(mode) {
  ...
  if (mode === "setup") {
    ...
    subtext.textContent = "Enter your EchoVault PIN — the one you use to unlock the app. This protects every memory you lock.";
  } else {
    title.textContent = "🔒 Enter PIN";
    subtext.textContent = "Enter your EchoVault PIN to unlock this memory.";
  }
...
function persistMemoryUnlock(id) {
...
async function submitMemoryLockPin() {
 ...
 if (!hasMemoryLockPin()) {
    // First-ever lock: this entry becomes the shared PIN (your main EchoVault PIN).
    const hash = await hashPIN(entered);
    localStorage.setItem("memoryLockPIN", hash);
    if (pendingLockAction) persistMemoryUnlock(pendingLockAction.targetId);
...
document.getElementById("memoryForm").addEventListener("submit", async function (e) {
   ...
   setMemories(memories);
    ...
    if (typeof checkAtticSecretTitleTrigger === "function") {
      checkAtticSecretTitleTrigger(title);
    }
    ...
    <div class="flex justify-between items-start">
        ...
        <div>
            ...
            <h4 class="font-semibold text-white text-lg">
                📖 ${(memory.locked) ? "Locked memory" : escapeHTML(memory.title)}
            </h4>
...
`;
  ...
  if (memoriesListMain) memoriesListMain.innerHTML = 
    ...
    <div onclick="${(memory.locked) ? `handleLockedCardTap(${memory.id})` : `viewMemory(${memory.id})`}" class="cursor-pointer bg-zinc-900 border border-zinc-700 hover:border-cyan-500/60 rounded-3xl overflow-hidden fade-in stagger-${(index % 5) + 1} transition-all">
...
<h3 class="text-lg font-bold text-white flex items-center gap-2">
                    ${memory.locked ? "🔒 Locked memory" : highlightMatch(memory.title, searchText)}
...
<div onclick="viewMemory(${memory.id})" class="cursor-pointer bg-zinc-900 border border-cyan-500/60 hover:border-cyan-400 rounded-3xl overflow-hidden mb-4 transition-all duration-300 hover:shadow-lg hover:shadow-cyan-500/10">
   ...
   ${memory.image ? `
    ...
    <div class="p-5">
        ...
        <h3 class="text-lg font-bold text-white leading-snug">${(memory.locked) ? "🔒 Locked memory" : escapeHTML(memory.title)}</h3>
        ${(memory.locked) ? "" : (excerpt ? `<p class="text-zinc-400 text-sm mt-1.5 leading-relaxed">${excerpt}</p>` : "")}
    ...
    onThisDayEl.innerHTML = "";
        ...
        <div class="relative border-l-4 border-cyan-500 pl-6 py-5 ml-3 fade-in">
            ...
            <div onclick="${(memory.locked) ? `handleLockedCardTap(${memory.id})` : `viewMemory(${memory.id})`}" class="cursor-pointer bg-zinc-900 border border-cyan-500/30 hover:border-cyan-400 rounded-3xl overflow-hidden transition-all">
...
const ACHIEVEMENT_UNLOCK_TUNES = {
    Memory:   { notes: [523.25, 659.25, 783.99], wave: "sine", stagger: 0.08, sustain: 0.4 },
    Writing:  { notes: [493.88, 587.33], wave: "sine", stagger: 0.15, sustain: 0.6 },
    Story:    { notes: [392.00, 493.88, 587.33, 659.25], wave: "sine", stagger: 0.25, sustain: 1.0 },
    Streak:   { notes: [659.25, 783.99, 880.00, 987.77, 1046.50], wave: "triangle", stagger: 0.05, sustain: 0.2 },
    Explorer: { notes: [523.25, 659.25, 783.99, 1046.50], wave: "sine", stagger: 0.1, sustain: 0.45 },
    Time:     { notes: [440.00, 440.00, 440.00, 523.25], wave: "square", stagger: 0.15, sustain: 0.1 },
    Category: { notes: [659.25, 880.00], wave: "triangle", stagger: 0.04, sustain: 0.15 },
    Secret:   { notes: [493.88, 466.16, 440.00], wave: "sine", stagger: 0.2, sustain: 0.7 }
};

function playUnlockSound(category) {
    try {
        const ctx = new (window.AudioContext || window.webkitAudioContext)();
        const tune = ACHIEVEMENT_UNLOCK_TUNES[category] || ACHIEVEMENT_UNLOCK_TUNES.Memory;
        tune.notes.forEach((freq, i) => {
            ...
            osc.type = tune.wave;
            const start = ctx.currentTime + i * tune.stagger;
            gain.gain.setValueAtTime(0, start);
            gain.gain.linearRampToValueAtTime(0.15, start + 0.02);
            gain.gain.exponentialRampToValueAtTime(0.001, start + tune.sustain);
            osc.start(start);
            osc.stop(start + tune.sustain);
        });
    } catch (e) {}
}
...
function checkAtticDoorUnlock() {
  // TEMPORARY: always unlock the Attic door for demo / testing
  return true;
}
...
function showAtticDoor() {
  ...
  if (localStorage.getItem("atticSetupComplete") === "true") {
    wireAtticDoorReturnTap();
  } else {
...
let atticDoorReturnTapWired = false;
function wireAtticDoorReturnTap() {
  const door = document.getElementById("atticDoorVisual");
  if (atticDoorReturnTapWired || !door) return;
  atticDoorReturnTapWired = true;

  door.addEventListener("pointerdown", handleAtticDoorPointerDown);
  door.addEventListener("pointerup", handleAtticDoorPointerUp);
}

const ATTIC_RHYTHM_TAP_COUNT = 15;
const ATTIC_RHYTHM_TAP_MAX_GAP = 700;
const ATTIC_RHYTHM_TAP_HOLD_MIN = 500;

let atticRhythmTapCount = 0;
let atticRhythmTapLastTime = 0;
let atticRhythmTapPointerDownAt = 0;
let atticSingleTapTimer = null;

function handleAtticDoorPointerDown() {
  if (atticSingleTapTimer) {
    clearTimeout(atticSingleTapTimer);
    atticSingleTapTimer = null;
  }
  atticRhythmTapPointerDownAt = Date.now();
}

function handleAtticDoorPointerUp() {
  if (atticResetWarningTriggered) {
    atticResetWarningTriggered = false;
    atticRhythmTapCount = 0;
    if (atticSingleTapTimer) {
      clearTimeout(atticSingleTapTimer);
      atticSingleTapTimer = null;
    }
  ...
  const now = Date.now();
  const heldFor = now - atticRhythmTapPointerDownAt;
  ...
  const gapBeforeThisPress = atticRhythmTapPointerDownAt - atticRhythmTapLastTime;
  const isFinalHold = heldFor >= ATTIC_RHYTHM_TAP_HOLD_MIN;

  if (atticRhythmTapCount > 0 && gapBeforeThisPress > ATTIC_RHYTHM_TAP_MAX_GAP) {
    atticRhythmTapCount = 0;
  }

  if (isFinalHold && atticRhythmTapCount >= ATTIC_RHYTHM_TAP_COUNT - 1) {
    atticRhythmTapCount = 0;
    if (typeof handleAtticTriggerMatch === "function") {
      handleAtticTriggerMatch("atticRequirePassword_rhythmTap");
    }
    return;
  }
  ...
  if (isFinalHold) {
    atticRhythmTapCount = 0;
    openAtticDoorPasswordModal();
    return;
  }

  atticRhythmTapCount++;
  atticRhythmTapLastTime = now;

  atticSingleTapTimer = setTimeout(() => {
    atticRhythmTapCount = 0;
    openAtticDoorPasswordModal();
  }, ATTIC_RHYTHM_TAP_MAX_GAP);
}

function openAtticDoorPasswordModal() {
  if (typeof openAtticReturnEntryModal === "function") {
    openAtticReturnEntryModal();
  }
}

// --- Full reset: hold the door for 10+ seconds ---
const ATTIC_RESET_HOLD_DURATION = 10000;
const ATTIC_RESET_HOLD_TOLERANCE = 20;

let atticResetWarningTriggered = false;
let atticResetHoldTimer = null;
let atticResetHoldStartX = 0;
let atticResetHoldStartY = 0;

function handleAtticResetHoldStart(e) {
  if (localStorage.getItem("atticSetupComplete") !== "true") return;
  atticResetHoldStartX = e.clientX;
  atticResetHoldStartY = e.clientY;

  atticResetHoldTimer = setTimeout(function () {
    atticResetHoldTimer = null;
    atticResetWarningTriggered = true;
    if (typeof openAtticResetWarningModal === "function") {
      openAtticResetWarningModal();
    }
  }, ATTIC_RESET_HOLD_DURATION);
}

function handleAtticResetHoldMove(e) {
  if (!atticResetHoldTimer) return;
  const dx = e.clientX - atticResetHoldStartX;
  const dy = e.clientY - atticResetHoldStartY;
  if (Math.sqrt(dx * dx + dy * dy) > ATTIC_RESET_HOLD_TOLERANCE) {
    clearTimeout(atticResetHoldTimer);
    atticResetHoldTimer = null;
  }
}

function handleAtticResetHoldEnd() {
  if (atticResetHoldTimer) {
    clearTimeout(atticResetHoldTimer);
    atticResetHoldTimer = null;
  }
}

let atticResetHoldWired = false;
function wireAtticResetHold() {
  const door = document.getElementById("atticDoorVisual");
  if (atticResetHoldWired || !door) return;
  atticResetHoldWired = true;

  door.addEventListener("pointerdown", handleAtticResetHoldStart);
  door.addEventListener("pointermove", handleAtticResetHoldMove);
  door.addEventListener("pointerup", handleAtticResetHoldEnd);
  door.addEventListener("pointercancel", handleAtticResetHoldEnd);
}
...
function startAtticDoorRhythm() {
  ...
  const machine = createAtticRhythmMachine({
    ...
    onSuccess: () => {
      door.classList.remove("ring-4", "ring-emerald-400/70", "scale-95");
      door.classList.remove("animate-[wiggle_0.5s_ease]");
      if (rhythmIntervalId) {
...
const ACHIEVEMENT_TOAST_FEEL = {
    Memory:   { durationMs: 500,  easing: "ease-out",    displayMs: 8000,  scaleIn: false },
    Writing:  { durationMs: 700,  easing: "ease-out",    displayMs: 8500,  scaleIn: false },
    Story:    { durationMs: 900,  easing: "ease-out",    displayMs: 11000, scaleIn: true  },
    Streak:   { durationMs: 350,  easing: "cubic-bezier(0.34, 1.56, 0.64, 1)", displayMs: 7000, scaleIn: false },
    Explorer: { durationMs: 550,  easing: "ease-out",    displayMs: 8500,  scaleIn: false },
    Time:     { durationMs: 500,  easing: "linear",      displayMs: 8000,  scaleIn: false },
    Category: { durationMs: 300,  easing: "ease-out",    displayMs: 7000,  scaleIn: false },
    Secret:   { durationMs: 1100, easing: "ease-in-out", displayMs: 11000, scaleIn: false }
};
...
function showAchievementToast(achievement) {
    ...
    const feel = ACHIEVEMENT_TOAST_FEEL[achievement.category] || ACHIEVEMENT_TOAST_FEEL.Memory;
    const isStory = achievement.category === "Story";

    toast.className =
        `fixed top-6 left-1/2 -translate-x-1/2 opacity-0 bg-zinc-900 border-2 ${colorClasses} text-white px-6 py-5 rounded-3xl shadow-2xl z-[999] flex items-start gap-4 max-w-sm -translate-y-[150%] transition-all`;
    toast.style.transitionDuration = `${feel.durationMs}ms`;
    toast.style.transitionTimingFunction = feel.easing;
...
function renderHomeRecentMemories() {
    ...
    container.innerHTML = memories.map(memory => `
        <div onclick="${(memory.locked) ? `handleLockedCardTap(${memory.id})` : `viewMemory(${memory.id})`}" class="cursor-pointer bg-zinc-900 border border-zinc-700 hover:border-cyan-500/60 rounded-3xl overflow-hidden transition-all">
